...
import { HardDriveUpload, ImageUp } from 'lucide-react';
...
export default function VideoUpload() {
  ...
  const handleVideoFileChange = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) {
      ...
      reader.onloadend = () => {
        const result = reader.result as string;
        setVideoFilePreview(result); // Set the data URL as the image preview
      };
      reader.readAsDataURL(file);
    }
  };
  ...
  return (
    <div className='flex flex-col w-screen justify-center items-center'>
      ...
      <div className='border p-4 rounded-lg border-input' >
        ...
        <input ref={titleRef} type="text" name="title" id='title' placeholder='video title' className=' bg-transparent px-4 py-1 rounded-lg border border-input ' />
        <textarea ref={descriptionRef} placeholder='video description' id='description' className='"flex min-h-[80px] w-full rounded-md border border-input bg-background px-3 py-2 text-sm ring-offset-background placeholder:text-muted-foreground focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 disabled:cursor-not-allowed disabled:opacity-50",'/>
          ...
          {/* thumbnail upload */}
              ...
              {/* video upload */}
                ...
                <div>
                  ...
                  isProcessing &&  <button className='px-8 rounded-xl animate-pulse hover:bg-blue-500 duration-300 py-2 bg-blue-400 text-lg font-semibold'>Uploading your video</button>
                } {
